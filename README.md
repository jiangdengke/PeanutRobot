...
- 选择一个或多个点位后立即导航
- 支持在主界面右侧查看和设置运行速度，默认速度为 `20`
- 根据单点或多点任务使用对应的导航速度配置
- 支持在主界面右侧查看和修改上游服务器 IP，默认地址为 `192.168.112.194`
- 支持本机 HTTP 任务控制
...
- 上游服务器 IP 默认为 `192.168.112.194`，支持在主界面右侧“上游地址”入口修改；保存后立即生效，不需要重启机器人或 App。详见 [上游地址设置](docs/upstream-server-settings.md)。
- 机器人 ID、任务 ID 和各上游端口目前仍是硬编码配置，后续应迁移到环境配置或运行时设置。
- 默认导航速度为 `20`，支持在主界面右侧设置 `20-100` 的运行速度；保存后从下一次出发开始生效，不需要重启机器人或 App。
- 默认锁屏密码为 `123456`。
