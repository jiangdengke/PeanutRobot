...
- 选择一个或多个点位后立即导航
- 主界面右侧提供“设置”入口，集中修改上游地址、运行速度、进入密码、锁屏设置，并可进入地图编辑和查看运行日志
- 支持查看和设置运行速度，默认速度为 `20`
- 根据单点或多点任务使用对应的导航速度配置
- 支持查看和修改上游服务器 IP，默认地址为 `192.168.112.194`
- 支持本机 HTTP 任务控制
...
- 主界面右侧“设置”入口的密码固定为 `0`，集中修改上游地址、运行速度、进入密码和锁屏设置，并可进入地图编辑和查看运行日志。详见 [设置入口](docs/settings-entry.md)。
- 上游服务器 IP 默认为 `192.168.112.194`，在设置内的“上游地址”修改；保存后立即生效，不需要重启机器人或 App。详见 [上游地址设置](docs/upstream-server-settings.md)。
- 机器人 ID、任务 ID 和各上游端口目前仍是硬编码配置，后续应迁移到环境配置或运行时设置。
- 默认导航速度为 `20`，在设置内可设置 `20-100` 的运行速度；保存后从下一次出发开始生效，不需要重启机器人或 App。
- 默认进入密码为 `123456`，用于锁屏解锁和长按地图进入编辑，可在设置内修改；该密码与设置入口密码 `0` 相互独立。
